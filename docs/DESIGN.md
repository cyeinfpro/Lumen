...
### V1.1+（按真实使用反馈决定）
- 如果朋友反复要求 → 加 Mixed 模式 / 图片对比滑块 / Collections
...
| **P2** | **砍掉所有计费 / 额度 / 充值** | §1 非目标、§4 删表/字段、§5 删接口、§6.8 删退款、§14 删 credits 显示、§22.7 改用量展示、§20 V1.0 改内测 |
...
是。但经过 §20 的重排，V0.2（3 周）已经是**可给自己用**的最小切片；V0.5（+5 周）已经是**可邀请制 Beta** 的切片。其余都是增量。
