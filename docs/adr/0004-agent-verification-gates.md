...
They also prove the server-owned safety budget, pre-prompt-only checkpoint
persistence, retained-text reset accounting, and bounded NDJSON event output.
...
The `truncated` scenario requires a `partial` result with
`agent_output_truncated` and a `turn.completed` length stop. Continuation is an
explicit server command; truncation is never presented as ordinary success.
