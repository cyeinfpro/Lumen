"""Bounded HTTP body, JSON, and SSE primitives for the image-job package."""
...
def _validate_json_scalar(current: Any, limits: JsonShapeLimits) -> None:
    if isinstance(current, str):
        if len(current) > limits.max_string_chars:
            raise HTTPException(
                status_code=400,
                detail=f"JSON string exceeds {limits.max_string_chars} characters",
            )
        return
...
def validate_json_shape(value: Any, limits: JsonShapeLimits) -> None:
    ...
    while stack:
        ...
        if isinstance(current, dict):
            _push_json_mapping_items(current, depth, limits, stack)
        elif isinstance(current, list):
            _push_json_array_items(current, depth, limits, stack)
        else:
            _validate_json_scalar(current, limits)
