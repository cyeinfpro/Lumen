...
上游图片 API 必须能接收供应商的 `Authorization: Bearer ...`。Lumen 调用
sidecar 时，标准 `Authorization` 只携带独立的 `IMAGE_JOB_SIDECAR_TOKEN`；
供应商 Bearer 通过内部请求头 `X-Lumen-Upstream-Authorization` 单独传入，
sidecar 只在执行任务时把它转发给上游。
...
- `stream_only` 完全不要求 sidecar URL/token。
- `auto` 只在选中的 Provider 启用 image jobs 时校验 sidecar；URL/token
  配置不可用会自动回退 stream。
- `image_jobs_only` 是严格模式，URL/token 缺失、格式错误或使用示例占位域名时
  会 fail-fast。
- URL 必须使用 `http` 或 `https`、包含 host，且不得包含凭证、query 或 fragment。
...
EnvironmentFile=/etc/image-job/image-job.env
...
在启动服务前创建仅服务账号可读的凭证文件：

```bash
sudo install -d -m 0750 /etc/image-job
sudo sh -c 'umask 077; printf "%s\n" "IMAGE_JOB_SIDECAR_TOKEN=<随机值>" > /etc/image-job/image-job.env'
sudo chmod 0600 /etc/image-job/image-job.env
```
...
2. POST /v1/image-jobs，Authorization 带 sidecar token，并用独立内部请求头传上游 Bearer。
...
SIDECAR_HEADERS = {
    "Authorization": "Bearer <IMAGE_JOB_SIDECAR_TOKEN>",
    "X-Lumen-Upstream-Authorization": "Bearer <UPSTREAM_API_KEY>",
}
...
def submit_image_job(endpoint: str, body: dict, request_type: str | None = None) -> str:
    ...
    resp = requests.post(
        ...
        headers={**SIDECAR_HEADERS, "Content-Type": "application/json"},
        json=payload,
        timeout=30,
    )
...
curl -sS https://example.com/v1/image-jobs \
  -H "Authorization: Bearer <IMAGE_JOB_SIDECAR_TOKEN>" \
  -H "X-Lumen-Upstream-Authorization: Bearer <UPSTREAM_API_KEY>" \
...
404            -> job_id 错误或任务已清理
403            -> sidecar token 无权访问该任务，或迁移期缺少旧任务对应的上游头
```
...
调用方没有传正确的 `Authorization: Bearer <IMAGE_JOB_SIDECAR_TOKEN>`。
sidecar 不接受无服务鉴权任务。

### POST 返回 400 且提示缺少上游凭证

新鉴权模式下，创建任务还必须传
`X-Lumen-Upstream-Authorization: Bearer <UPSTREAM_API_KEY>`。该值不能与
sidecar token 混用。
...
X-Lumen-Upstream-Authorization header
