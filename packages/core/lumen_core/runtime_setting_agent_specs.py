...
AGENT_SETTINGS: tuple[SettingSpec, ...] = (
    ...
    SettingSpec(
        key="agent.max_web_search_calls",
        description="Maximum web-search tool calls in one Agent run.",
        sensitive=False,
        parser=int,
        env_fallback="AGENT_MAX_WEB_SEARCH_CALLS",
        min_value=0,
        max_value=8,
    ),
    SettingSpec(
        key="agent.max_file_tool_calls",
        description="Maximum virtual file-tool calls in one Agent run.",
        sensitive=False,
        parser=int,
        env_fallback="AGENT_MAX_FILE_TOOL_CALLS",
        min_value=0,
        max_value=32,
    ),
    SettingSpec(
        key="agent.max_tool_calls",
        description="Maximum aggregate first-party tool calls in one Agent run.",
        sensitive=False,
        parser=int,
