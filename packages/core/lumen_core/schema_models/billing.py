...
class AdminWalletAdjustIn(BaseModel):
    ...
    idempotency_key: str | None = Field(
        ...
        max_length=96,
        description=(
            "Per-operation key generated by the client (one UUID per form "
            "submission; retries of the same submission reuse the same key). "
            "Without a key the server derives one from amount+reason and "
            "rejects a second identical adjustment instead of silently "
            "replaying the first."
        ),
    )
