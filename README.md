...
| Infra | PostgreSQL 16, Redis 7, Docker Compose, systemd, Nginx |
...
管理后台可以调整 `telegram.*` 设置并通过 `/admin/telegram/restart` 发布 Redis 控制消息，让 Bot clean exit 后由 systemd `Restart=always` 拉起。
...
- `deploy/systemd/lumen-api.service`
- `deploy/systemd/lumen-worker.service`
- `deploy/systemd/lumen-web.service`
- `deploy/systemd/lumen-tgbot.service`
- `deploy/systemd/lumen-backup.{service,timer}`
- `deploy/systemd/lumen-health-watchdog.{service,timer}`
...
```text
/opt/lumen          # 代码
/opt/lumen/.env    # 统一环境变量
/opt/lumendata     # storage + backup
```
...
- `proxy_buffering off` 用于 SSE 和 API
- `proxy_request_buffering off` 用于大图上传
- `proxy_read_timeout 600s` 或更高
- `gzip off`，避免 SSE 被压缩缓冲
...
# 卸载向导：停止 systemd / 本地进程 / 容器，并禁用 Lumen nginx 反代；数据默认保留
...
# 恢复指定 timestamp 的备份
bash scripts/restore.sh 20260424-123000
```
...
systemd timer 默认每 4 小时备份一次，保留最近 `MAX_KEEP=40` 份。
...
```bash
journalctl -u lumen-worker -n 200 --no-pager
redis-cli keys 'task:*:lease'
```
...
- 查看日志：`journalctl -u lumen-tgbot -n 200 --no-pager`。
...
- `deploy/README.md`：systemd、Nginx、image-job 部署说明。
