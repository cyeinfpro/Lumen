...
export function ImageModelSelect() {
  ...
  return (
    <label className="grid min-w-40 shrink-0 gap-1 text-[var(--fg-0)]">
      <span className="type-overline text-[var(--fg-2)]">生图模型</span>
      <Select aria-label="生图模型" value={model} onChange={(event) => setModel(normalizeImageModel(event.target.value))}>
