  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col gap-3", className)}>
      ...
      <header className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 border-b border-[var(--border)] pb-2 md:hidden">
        <div className="type-caption min-w-0 flex-1">
      ...
      <div className="grid min-h-0 flex-1 gap-4 md:grid-cols-[116px_minmax(0,1fr)] xl:grid-cols-[124px_minmax(0,1fr)]">
            ...
            {SOURCE_FILTERS.map(([value, label]) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "type-control group relative flex min-h-9 cursor-pointer items-center justify-between border-b border-[var(--border)] py-1.5 transition-colors",
                    active
                      ? "text-[var(--fg-0)]"
        ...
        <main className="flex min-h-0 min-w-0 flex-col gap-3">
          ...
          <div className="sticky top-0 z-[var(--z-tabbar)] -mx-3 flex items-center gap-2 bg-[var(--bg-0)]/95 px-3 py-2 shadow-[var(--shadow-1)] backdrop-blur-xl md:hidden">
            ...
            <button
              ...
              className={cn(
                "type-control inline-flex min-h-11 shrink-0 cursor-pointer items-center gap-1.5 rounded-[var(--radius-control)] border px-3 transition-colors focus-visible:outline-none focus-visible:shadow-[var(--ring)]",
                activeFilterCount > 0
            ...
            <div className="flex min-w-0 items-center gap-3 border-b border-[var(--border)] pb-2 xl:col-span-2">
              <div className="relative w-full min-w-0 max-w-md">
                ...
                <input
                  ...
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder="搜索名称、标签"
                  className="control-shell type-body-sm h-10 w-full min-w-0 pl-9 pr-10 text-[var(--fg-0)] outline-none transition-[border-color,box-shadow,background-color] placeholder:text-[var(--fg-2)] focus:border-[var(--accent)]/60 focus:ring-2 focus:ring-[var(--accent)]/20"
                  aria-label="搜索风格"
                />
                {query ? (
                  <button
              ...
              <div className="ml-auto flex shrink-0 items-center gap-1.5">
                <p className="type-caption hidden max-w-[180px] truncate xl:block">
                  {syncSummary}
                </p>
                {renderBrowserActions()}
              </div>
          ...
          <div className="min-h-0 flex-1">
            {libraryQuery.isPending ? (
              <div className="type-body-sm flex h-64 items-center justify-center gap-2">
                <Spinner size={20} />
                加载中
              </div>
            ) : items.length === 0 ? (
                ...
                {deletableIds.length > 0 ? (
                  <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-y border-[var(--border)] py-1.5">
                    <button
                      ...
                      className="type-control inline-flex min-h-11 min-w-0 items-center gap-2 text-[var(--fg-1)] transition-colors hover:text-[var(--fg-0)] md:h-8 md:min-h-0"
                    >
                      {allVisibleSelected ? (
                    ...
                    {selectedDeletableIds.length > 0 ? (
                      <div className="flex max-w-full flex-wrap items-center justify-end gap-2">
                        <button
                          ...
                          className="type-control inline-flex min-h-11 items-center px-2 text-[var(--fg-2)] transition-colors hover:text-[var(--fg-0)] md:h-8 md:min-h-0"
                        >
                          取消
                        </button>
                        <Button
...
function ChipRowGroup({
  ...
  return (
    <div className="flex min-w-0 items-start gap-2.5">
      <p className="type-label mt-1.5 w-[58px] shrink-0">
        {label}
      </p>
...
function Chip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "type-control group relative inline-flex min-h-11 min-w-11 shrink-0 cursor-pointer items-center justify-center px-1 py-1 transition-colors focus-visible:outline-none focus-visible:shadow-[var(--ring)] md:min-h-9 md:min-w-9",
...
function EmptyBrowser() {
  return (
    <div className="border-y border-[var(--border)] py-16 md:py-20">
      <div className="grid gap-3">
        <h4 className="type-page-title">当前筛选没有风格</h4>
  ...
  return (
    <article className="group relative">
      ...
      <MediaControlButton
        ...
        className={cn(
          "absolute left-2 top-2 z-[var(--z-header)] md:h-8 md:w-8",
          selected
      ...
      <button
        ...
        className="relative block aspect-square w-full cursor-pointer overflow-hidden rounded-[var(--radius-card)] bg-[var(--bg-2)] focus-visible:outline-none focus-visible:shadow-[var(--ring)]"
      >
        <Image
          src={previewUrl}
          alt={item.title}
        ...
        />
        <span className="type-caption absolute right-2 top-2 inline-flex items-center rounded-[var(--radius-control)] bg-[var(--media-control-bg)] px-2 py-1 text-[var(--media-control-fg)]">
      ...
      <div className="mt-2 grid min-w-0 gap-0.5">
        <p className="type-body-sm line-clamp-1 min-w-0 break-words font-medium text-[var(--fg-0)]">
          {item.title}
        </p>
        {item.mood ? (
          <p className="type-caption line-clamp-1 min-w-0 break-words">
            {item.mood}
          </p>
        ) : null}
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
      <button
        ...
        aria-label={label}
        className={cn(
          "type-control inline-flex min-h-11 cursor-pointer items-center gap-1 transition-colors disabled:cursor-not-allowed disabled:opacity-50 md:h-7 md:min-h-0",
          confirmingDelete
            ? "text-[var(--danger)]"
...
function MobileFilterSheet({
  ...
  return (
    <div
      className="fixed inset-0 z-[var(--z-dialog)] flex items-end bg-[var(--surface-scrim)] backdrop-blur-sm mobile-dialog-shell md:hidden"
      onMouseDown={(event) => {
