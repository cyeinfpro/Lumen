    ...
    return (
      <div className="flex flex-col gap-2.5 relative">
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 px-1">
          <p className="type-body-sm truncate flex-1 min-w-0 basis-[10rem]">
            {gen.prompt}
          </p>
          <StageTicker gen={gen} />
        </div>
  ...
  if (gen.status === "failed") {
    return (
      ...
      >
        ...
        >
          <p className="type-card-title text-danger">生成失败</p>
          {/* MED #11：移动端不要 text-xs，不然误触屏小屏几乎看不清 */}
          <p className="type-body-sm md:text-xs text-[var(--fg-2)] text-center max-w-full md:max-w-md leading-relaxed break-words [overflow-wrap:anywhere]">
            {failureMessage}
          </p>
          <Button
            type="button"
        ...
        <p className="type-body-sm px-1 truncate">{gen.prompt}</p>
      </motion.div>
    );
  }
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 px-1">
          ...
          <div className="flex items-center gap-2 shrink-0 type-caption font-mono tabular-nums">
            {elapsed != null && <span>{elapsed}s</span>}
            {elapsed != null && <span className="text-[var(--fg-3)]">·</span>}
  ...
  if (gen.status === "canceled") {
    return (
      <div className="flex flex-col gap-2 px-1 opacity-70">
        ...
        <p className="type-body-sm truncate">{gen.prompt}</p>
      </div>
    );
  }
    ...
    <div className={className}>
      <Button
        ...
        className={cn(
          ...
          "hover:border-danger-border hover:text-danger",
        )}
      >
        {busy ? "取消中" : copy.action.cancel}
      </Button>
      {error && (
        <p className="absolute mt-1 text-[10px] text-danger max-w-[200px]">
          {error}
        </p>
      )}
    </div>
