...
// 海报工作流步骤侧栏：mirror StepRail，但 STEPS = POSTER_STEPS（7 个）。
// - 顶部 progress 数字
// - 列表 N°NN 序号 + 节点圆 + 中文标题 + 状态色
// - 当前步：amber dot pulse + 琥珀文字
// - 移动条：横向滚 chip + 大数字 progress

import { CheckCircle2 } from "lucide-react";

import { cn } from "@/lib/utils";
import type { WorkflowRun } from "@/lib/apiClient";
import { POSTER_STEPS, POSTER_STEP_INDEX } from "../types";
import { stepOf } from "../utils";

function posterProgress(workflow: WorkflowRun): number {
  const total = POSTER_STEPS.length;
  const currentIndex = POSTER_STEP_INDEX[workflow.current_step] ?? 0;
  if (workflow.status === "completed") return 1;
  return Math.min(0.99, currentIndex / Math.max(1, total - 1));
}

export function PosterStepRail({ workflow }: { workflow: WorkflowRun }) {
  const currentIndex = POSTER_STEP_INDEX[workflow.current_step] ?? 0;
  const progress = posterProgress(workflow);
      ...
      <div>
        ...
        <div className="mt-2 flex items-baseline gap-3">
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--fg-2)]">
            % · {currentIndex + 1} / {POSTER_STEPS.length}
          </span>
        </div>
      ...
      <ol className="grid gap-0">
        {POSTER_STEPS.map((step, index) => {
          ...
          return (
            <li
              key={step.key}
              className="relative grid grid-cols-[24px_minmax(0,1fr)] gap-x-3 py-2.5"
            >
              <span
                className={cn(
...
export function MobilePosterStageStrip({ workflow }: { workflow: WorkflowRun }) {
  const currentIndex = POSTER_STEP_INDEX[workflow.current_step] ?? 0;
  const progress = posterProgress(workflow);
  return (
          ...
          return (
            ...
            >
              ...
              {isCurrent ? (
                <span
                  aria-hidden
                  className="ml-1 inline-block h-1 w-1 rounded-full bg-[var(--amber-400)] animate-[lumen-pulse-soft_1800ms_ease-in-out_infinite]"
                />
              ) : null}
            </span>
          );
