...
export function createStoryboard(body: StoryboardCreateIn): Promise<StoryboardRun> {
...
export function deleteStoryboard(id: string): Promise<{ ok: boolean }> {
  return apiFetch<{ ok: boolean }>(`/storyboards/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
...
export function createStoryboardAsset(
  ...
  return apiFetch<StoryboardRun>(`/storyboards/${encodeURIComponent(storyboardId)}/assets`, {
    method: "POST",
    body: JSON.stringify(body),
  });
}

export function patchStoryboardAsset(
  storyboardId: string,
  stepId: string,
...
export function createStoryboardShot(
  ...
  return apiFetch<StoryboardRun>(`/storyboards/${encodeURIComponent(storyboardId)}/shots`, {
    method: "POST",
    body: JSON.stringify(body),
  });
}
...
export function submitAllStoryboardShots(storyboardId: string): Promise<StoryboardRun> {
  return apiFetch<StoryboardRun>(
...
export function assembleStoryboard(storyboardId: string): Promise<StoryboardRun> {
  return apiFetch<StoryboardRun>(
...
export function listAdminBillingAudit(
  opts: { event_type?: string | null; limit?: number } = {},
): Promise<AdminBillingAuditEventOut[]> {
  const qs = new URLSearchParams();
  if (opts.event_type) qs.set("event_type", opts.event_type);
  if (opts.limit != null) qs.set("limit", String(opts.limit));
  const suffix = qs.toString() ? `?${qs.toString()}` : "";
  return apiFetch<AdminBillingAuditEventOut[]>(`/admin/billing/audit${suffix}`);
}
