...
const CODE_TITLE: Record<string, string> = {
  ...
  NO_ACTIVE_API_KEY: "需要重新绑定 API 密钥",
...
const CODE_DESC: Record<string, string> = {
  ...
  csrf_failed: "刷新页面后再试",
  quota_exceeded: "服务器繁忙，稍后重试",
  upstream_error: "服务暂时不可用，稍后再试",
  prompt_too_long: "精简提示词后再发送",
  invalid_request: "检查输入内容后重试",
  validation_error: "检查输入内容是否合规",
  FILE_SEARCH_NOT_CONFIGURED: "先配置文件检索库，或关闭文件检索后再发送",
  file_search_not_configured: "先配置文件检索库，或关闭文件检索后再发送",
  tool_loop_truncated: "为防止工具循环失控，本轮已停止，调整问题后重试",
  cancelled: "本次请求已取消，未产生新的输出",
  canceled: "本次请求已取消，未产生新的输出",
  not_found: "目标可能已被删除或不存在",
  client_exception: "客户端发生异常，刷新后再试",
  INSUFFICIENT_BALANCE: "先进入钱包兑换或联系管理员充值",
  insufficient_credits: "先进入钱包兑换或联系管理员充值",
  WALLET_FROZEN: "充值或联系管理员处理后再继续使用",
  WALLET_HAS_ACTIVE_HOLDS: "先取消或等待进行中的任务结束，再切换账号模式",
  NO_ACTIVE_API_KEY: "到设置里的 API 密钥 重新绑定一张可用的密钥",
  ACCOUNT_MODE_FORBIDDEN: "钱包账号和 BYOK 账号入口互斥，切换到对应入口",
  ACCOUNT_NOT_WALLET: "BYOK 账号不能做钱包调账",
  CODE_NOT_FOUND: "检查兑换码是否输入完整",
  ...
  REDEMPTION_SECRET_NOT_CONFIGURED: "先在管理后台配置兑换码 secret",
  BOOTSTRAP_INCOMPLETE: "管理员需要先完成计费初始化",
  BILLING_DISABLED: "联系管理员开启计费功能后再兑换",
  ALREADY_REVOKED: "这张兑换码此前已经被撤销",
  THRESHOLDS_PRICING_MISMATCH: "确保每个尺寸档位都有启用的价格规则",
};
