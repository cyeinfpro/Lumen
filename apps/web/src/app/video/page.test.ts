...
const source = [
  ...
  taskModelSource,
  taskUiSource,
  readFileSync(new URL("../../lib/videoEventSnapshot.ts", import.meta.url), "utf8"),
].join("\n");
...
const taskModel = (await import(taskModelUrl.href)) as typeof import("./video-task-model");
...
test("video reference identity, labels, limits, and asset ids are stable", () => {
  ...
  equal(
    referenceDomain.referenceRefIndex(" REF:VIDEO:3 ", "video"),
    3,
  );
...
test("video reference aliases serialize to anchors and display round trips", () => {
  ...
  equal(
    referenceDomain.referenceDisplayToken(videoReference),
    "@视频1",
  );
  ok(
    referenceDomain
      .referenceDisplayAliases(imageReference)
      .includes("@图 1"),
  );
  ok(
    referenceDomain
  ...
  equal(
    referenceDomain.normalizePromptReferenceMentions(
      "第一张图片作为主体",
      [imageReference],
    ),
...
test("video prompt enhancement and request serialization preserve anchors", () => {
  ...
  deepEqual(
    referenceDomain.referencePayloadForVideoAction(
      "reference",
      testReferences,
    ),
    [
...
test("video task model preserves status, elapsed, and error semantics", () => {
  ...
  equal(
    taskModel.isTerminalVideo(taskFixture({ status: "succeeded" })),
    true,
  );
  equal(
...
test("video retry fences the original request while accepting a new task id", () => {
  ...
  match(retrySource, /scheduleGenerationRefresh\(gen\.id, \{ delayMs: 800 \}\)/);
...
test("video uploads are fenced to the current draft and upload epoch", () => {
  ...
  match(source, /isVideoRequestFenceCurrent\(draftFenceRef\.current, request\.draftFence\)/);
  match(source, /uploadImage\(request\.file, \{ signal: request\.controller\.signal \}\)/);
  match(source, /uploadReferenceVideo\(\s*request\.file,\s*request\.controller\.signal/);
  match(source, /switchDraftContext\(item\.id, item\.action\)/);
  match(source, /cancelFirstFrameUpload\(\)/);
  match(source, /cancelReferenceUpload\(\)/);
  match(source, /if \(isAbortError\(err\) \|\| !isCurrentReferenceUpload\(request\)\) return/);
});
...
test("video prompt enhancement cannot write into a later draft", () => {
  ...
  match(source, /isVideoRequestFenceCurrent\(draftFenceRef\.current, requestDraftFence\)/);
  match(
    source,
