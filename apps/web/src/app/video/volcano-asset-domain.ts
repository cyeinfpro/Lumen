...
export function volcanoOperationStageMessage(stage?: string | null): string {
  ...
  const messages: Record<string, string> = {
    ...
    retrying_upstream: "火山服务波动，后台自动重试中",
    reconciling_submit: "提交结果确认中，后台会自动继续",
    recovery_queued: "任务已由后台自愈接管",
    waiting_intent_lock: "同一素材正在确认提交结果",
    waiting_submit_slot: "已进入火山提交队列，按额度自动发送",
    waiting_rate_limit: "火山上游限流，后台自动重试",
    submitting: "火山素材库提交中",
    completed: "已提交到火山素材库",
    failed: "后台任务失败",
  };
