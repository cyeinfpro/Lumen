                    ...
                    return (
                      <Fragment key={event.id}>
                        ...
                        >
                          ...
                          <td className="py-3 px-3 font-mono type-caption tabular-nums whitespace-nowrap text-[var(--fg-1)]">
                          ...
                          <td className="py-3 px-3 text-right font-mono type-caption tabular-nums text-[var(--fg-1)] whitespace-nowrap">
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="type-body-sm text-[var(--fg-0)]">
                              {eventKindLabel(event)}
                            </span>
                            <StatusBadge status={event.status} />
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 type-caption md:grid-cols-4">
...
function ProviderCell({ event }: { event: AdminRequestEventOut }) {
  ...
  if (hasLiveLaneDetail) {
    return (
      <div className="flex max-w-[220px] flex-col gap-1 type-caption">
        {lanes.map((lane, idx) => (
  ...
  return (
    <div className="flex max-w-[190px] flex-col gap-1 type-caption">
...
function ImagesButton({ event }: { event: AdminRequestEventOut }) {
  if (event.images.length === 0) {
    return <span className="type-caption text-[var(--fg-2)]">—</span>;
  }
  ...
  return (
    <button
      ...
      disabled={!canOpen}
      className="inline-flex min-h-[36px] items-center justify-center gap-1.5 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--bg-2)] px-2.5 type-caption text-[var(--fg-0)] transition-colors hover:bg-[var(--bg-3)] disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/25"
    ...
    >
      ...
      查看
      <span className="rounded bg-[var(--bg-3)] px-1.5 py-0.5 font-mono type-caption text-[var(--fg-1)]">
        {event.images.length}
      </span>
      {outputCount > 0 && (
        <span className="rounded bg-[var(--accent)]/15 px-1.5 py-0.5 type-caption text-[var(--accent)]">
          输出 {outputCount}
        </span>
      )}
    </button>
    ...
    <div className="space-y-4 rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--bg-0)]/60 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border-subtle)] pb-3">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          <StatusBadge status={event.status} />
          {outputCount > 0 && (
            <span className="inline-flex items-center gap-1 rounded-[var(--radius-control)] border border-[var(--accent)]/20 bg-[var(--accent)]/10 px-1.5 py-0.5 type-caption text-[var(--accent)]">
              <ImageIcon className="h-3 w-3" />
              输出 {outputCount}
            </span>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 type-caption text-[var(--fg-2)]">
      ...
      {event.error_message && (
        <div>
          ...
          <p className="max-h-28 overflow-auto whitespace-pre-wrap rounded-[var(--radius-control)] border border-danger-border bg-danger-soft p-3 type-caption leading-relaxed text-danger">
            {event.error_message}
          </p>
        </div>
      )}
          ...
          <div className="flex flex-wrap gap-2">
            {upstreamEntries.map(([key, value]) => (
              <span
                ...
                className="inline-flex max-w-full items-center gap-1 rounded-[var(--radius-card)] border border-[var(--border-subtle)] bg-[var(--bg-2)] px-2 py-1 type-caption text-[var(--fg-1)]"
              >
...
function Detail({
  ...
  return (
    <div className="min-w-0">
      ...
      <div
        className={cn(
          "mt-1 break-words type-caption text-[var(--fg-1)]",
...
function MiniField({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-[var(--radius-card)] border border-[var(--border-subtle)] bg-[var(--bg-2)] px-2 py-1.5">
      ...
      <div
        className="mt-0.5 line-clamp-2 break-words type-caption text-[var(--fg-1)]"
        title={value || "—"}
      >
        {value || "—"}
      </div>
