...
def test_classify_inflight_lane_image2_routes() -> None:
    assert (
        queue.classify_inflight_lane("image2", "images/generations") == "lane_a"
    )
    assert queue.classify_inflight_lane("image2_direct", None) == "lane_a"


def test_classify_inflight_lane_responses_routes() -> None:
    assert queue.classify_inflight_lane("responses", None) == "lane_b"
    assert queue.classify_inflight_lane("responses_fallback", None) == "lane_b"


def test_classify_inflight_lane_image_jobs_endpoint_split() -> None:
    assert (
        queue.classify_inflight_lane("image_jobs", "image-jobs:generations")
        == "lane_a"
    )
    assert (
        queue.classify_inflight_lane("image_jobs", "image-jobs:responses")
        == "lane_b"
    )
    # 没 endpoint 信息时退化到 lane_a，不让事件丢
    assert queue.classify_inflight_lane("image_jobs", None) == "lane_a"


def test_classify_inflight_lane_unknown_route_falls_back_to_lane_a() -> None:
    assert queue.classify_inflight_lane(None, None) == "lane_a"
    assert queue.classify_inflight_lane("", "") == "lane_a"
    assert queue.classify_inflight_lane("weird", None) == "lane_a"
...
async def test_inflight_set_fields_writes_hash_with_ttl() -> None:
    redis = _HashRedis()
    await queue.inflight_set_fields(
        redis,
        "task-1",
        {"mode": "single", "provider": "alpha"},
        services=services,
    )
    snap = await redis.hgetall(queue.image_inflight_key("task-1"))
    assert snap["mode"] == "single"
    assert snap["provider"] == "alpha"
    # updated_at 是字符串的整数毫秒
    assert snap["updated_at"].isdigit()
    assert redis.expires[queue.image_inflight_key("task-1")] >= 60


@pytest.mark.asyncio
async def test_inflight_set_fields_skips_empty_payload() -> None:
    redis = _HashRedis()
    await queue.inflight_set_fields(redis, "task-1", {}, services=services)
    await queue.inflight_set_fields(
        redis,
        "task-1",
        {"x": "", "y": None},
...
async def test_inflight_set_fields_dual_race_lane_round_trip() -> None:
    ...
    raw = await redis.hgetall(queue.image_inflight_key("task-1"))
...
async def test_inflight_clear_deletes_key() -> None:
    ...
    await queue.inflight_clear(redis, "task-1", services=services)
    assert queue.image_inflight_key("task-1") in redis.deletes
    assert redis.hashes == {}
