...
class _TrackedAsyncClient(httpx.AsyncClient):
    ...
    def __init__(
        self,
        *args: Any,
        runtime_services: UpstreamServices,
        **kwargs: Any,
    ) -> None:
    ...
    def stream(self, *args: Any, **kwargs: Any) -> Any:
        return self._runtime_services.lifecycle.TrackedStreamContext(
            self, super().stream(*args, **kwargs)
        )


async def _resolve_timeout_config(
    *,
    ...
    async def _resolve_float(spec_key: str, fallback: float) -> float:
        try:
            raw = await services.infrastructure.resolve(spec_key)
    ...
    return services.lifecycle.TimeoutConfig(
...
def _build_client(
    ...
    *,
    ...
    services = _runtime_services(runtime)
    settings = services.infrastructure.settings
    timeout_config = timeout_config or services.lifecycle.TimeoutConfig(
    ...
    if pinned_target is not None:
        client_kwargs["transport"] = (
            services.infrastructure.pinned_async_http_transport(
                pinned_target
            )
        )
    if runtime is not None:
        client_kwargs["runtime_services"] = services
    return services.lifecycle.TrackedAsyncClient(**client_kwargs)
...
def _cache_proxied_client(
    ...
    *,
    ...
    while len(cache) > services.core.PROXIED_CLIENT_CACHE_MAX:
...
async def _delayed_aclose(
    ...
    *,
    ...
    try:
        await asyncio.sleep(
            services.core.PROXIED_CLIENT_CLOSE_DELAY_SECONDS
            if delay is None
            else delay
        )
        ...
        if callable(wait_until_idle):
            try:
                await wait_until_idle(
                    services.core.PROXIED_CLIENT_IDLE_CLOSE_TIMEOUT_SECONDS
                )
            except asyncio.TimeoutError:
                services.infrastructure.logger.warning(
                    "timed out waiting for retired upstream client to idle"
                )
        await services.lifecycle.aclose_client_cancel_safe(client)
...
def _schedule_delayed_aclose(
    client: httpx.AsyncClient,
    *,
    runtime: ImageUpstreamRuntime | None = None,
) -> asyncio.Task[None]:
    services = _runtime_services(runtime)
    services.core.retired_clients.add(client)
    task = asyncio.create_task(services.lifecycle.delayed_aclose(client))
    services.core.retired_client_close_tasks.add(task)

    def _discard_retired_client(done: asyncio.Task[None]) -> None:
        services.core.retired_client_close_tasks.discard(done)
        services.core.retired_clients.discard(client)

    task.add_done_callback(_discard_retired_client)
    return task


async def _close_retired_clients_now(
    *,
    runtime: ImageUpstreamRuntime | None = None,
) -> None:
    services = _runtime_services(runtime)
    tasks = list(services.core.retired_client_close_tasks)
    clients = list(services.core.retired_clients)
    for task in tasks:
        task.cancel()
    if tasks:
        await asyncio.gather(*tasks, return_exceptions=True)
    services.core.retired_client_close_tasks.difference_update(tasks)
    services.core.retired_clients.difference_update(clients)
...
async def _get_client(
    ...
    *,
    ...
    if proxy_url:
        ...
        evicted: list[httpx.AsyncClient] = []
        async with services.core.client_lock:
            client = services.core.proxied_clients.get(key)
            if client is not None:
                services.core.proxied_clients.move_to_end(key)
                return client
            client = services.lifecycle.build_client(
                timeout_config, proxy_url=proxy_url
            )
            evicted = services.lifecycle.cache_proxied_client(
                services.core.proxied_clients,
                key,
                client,
            )
        for evicted_client in evicted:
            services.lifecycle.schedule_delayed_aclose(evicted_client)
        return client
    if pinned_target is not None:
        ...
        return client
    if (
        services.core.client is None
        or services.core.client_timeout_config != timeout_config
    ):
        async with services.core.client_lock:
            if (
                services.core.client is None
                ...
                retired_client = services.core.client
                services.core.client = (
                    services.lifecycle.build_client(timeout_config)
                )
                services.core.client_timeout_config = timeout_config
                if retired_client is not None:
                    services.lifecycle.schedule_delayed_aclose(
                        retired_client
                    )
    shared_client = services.core.client
...
async def _get_images_client(
    ...
    *,
    ...
    if proxy_url:
        key = (timeout_config, proxy_url)
        evicted: list[httpx.AsyncClient] = []
        async with services.core.images_client_lock:
            client = services.core.proxied_images_clients.get(key)
            if client is not None:
                services.core.proxied_images_clients.move_to_end(key)
                return client
            client = services.lifecycle.build_images_client(
                timeout_config,
                proxy_url=proxy_url,
            )
            evicted = services.lifecycle.cache_proxied_client(
                services.core.proxied_images_clients,
                key,
                client,
            )
        for evicted_client in evicted:
    ...
    if pinned_target is not None:
        ...
        return client
    if (
        services.core.images_client is None
        or services.core.images_client_timeout_config != timeout_config
    ):
        async with services.core.images_client_lock:
            if (
                services.core.images_client is None
                or services.core.images_client_timeout_config
                != timeout_config
            ):
                retired_client = services.core.images_client
                services.core.images_client = (
                    services.lifecycle.build_images_client(timeout_config)
                )
                services.core.images_client_timeout_config = timeout_config
                if retired_client is not None:
                    services.lifecycle.schedule_delayed_aclose(
                        retired_client
                    )
    shared_client = services.core.images_client
    assert shared_client is not None
    return shared_client
...
async def close_client(*, runtime: ImageUpstreamRuntime) -> None:
    ...
    await services.lifecycle.close_retired_clients_now()

    async with services.core.client_lock:
        clients: list[httpx.AsyncClient] = []
        if services.core.client is not None:
            clients.append(services.core.client)
            services.core.client = None
            services.core.client_timeout_config = None
        clients.extend(services.core.proxied_clients.values())
        services.core.proxied_clients.clear()
    ...
    async with services.core.images_client_lock:
        ...
        if services.core.images_client is not None:
            image_clients.append(services.core.images_client)
            services.core.images_client = None
            services.core.images_client_timeout_config = None
        image_clients.extend(services.core.proxied_images_clients.values())
        services.core.proxied_images_clients.clear()
        image_clients.extend(_pinned_images_clients.values())
        _pinned_images_clients.clear()
    for client in image_clients:
        await services.lifecycle.aclose_client_cancel_safe(client)
    await services.infrastructure.close_provider_proxy_tunnels()
