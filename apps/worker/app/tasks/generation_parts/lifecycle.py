...
async def raise_if_generation_interrupted(
    ...
    if lease_lost.is_set():
        raise generation_ports()._LeaseLost(f"generation lease lost {reason}")
    if await generation_ports()._is_cancelled(redis, task_id):
        raise generation_ports()._TaskCancelled(reason)
...
async def settle_existing_generated_image(
    ...
    *,
    ...
    if await generation_ports()._is_cancelled(redis, task_id):
        ...
        failure_delivery = generation_ports()._stage_generation_event(
            ...
            generation_ports().EV_GEN_FAILED,
            {
                ...
                "code": generation_ports().EC.CANCELLED.value,
    ...
    generation_ports()._ensure_generation_updated(result, task_id, generation.attempt)
    message = await session.get(generation_ports().Message, message_id)
    if message is not None and message.status not in (
        generation_ports().MessageStatus.SUCCEEDED,
        generation_ports().MessageStatus.CANCELED,
    ):
        message.status = generation_ports().MessageStatus.SUCCEEDED
            ...
            "images": [
                {
                    ...
                    "url": generation_ports().storage.public_url(
    ...
    try:
        ...
        generation_ports().task_duration_seconds.labels(
            kind="generation",
            outcome=generation_ports().safe_outcome("succeeded"),
        ).observe(duration)
