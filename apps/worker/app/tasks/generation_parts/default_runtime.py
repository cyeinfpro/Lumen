"""Generation worker composition and arq execution support.

Stateful helpers live in ``generation_parts`` and receive explicit runtime ports.
"""
...
from ...upstream_parts.image_dispatch import (
    edit_image,
    generate_image,
    image_endpoint_kind_for_engine as _image_endpoint_kind_for_engine,
)
from . import diagnostics as _generation_diagnostics
from . import image_artifact_contracts as _image_artifacts
from . import event_delivery as _generation_event_delivery
from . import lease as _generation_lease
from . import lifecycle as _generation_lifecycle
from . import persistence as _generation_persistence
from . import postprocess as _generation_postprocess
from . import queue as _generation_queue
from . import queue_claim as _generation_queue_claim
from . import references as _generation_references
from . import request_options as _generation_request_options
from . import retry_state as _generation_retry_state
from . import runner as _generation_runner
from . import workflow_hooks as _generation_workflow_hooks
from ..state import is_generation_terminal

logger = logging.getLogger(__name__)
_tracer = get_tracer("lumen.worker.generation")

_ALLOWED_UPSTREAM_IMAGE_FORMATS = _image_artifacts.ALLOWED_UPSTREAM_IMAGE_FORMATS
_GeneratedImageInspection = _image_artifacts.GeneratedImageInspection
_ImageVariantBundle = _image_artifacts.ImageVariantBundle
_MAX_UPSTREAM_IMAGE_SIDE = _image_artifacts.MAX_UPSTREAM_IMAGE_SIDE
_PostprocessedGeneratedImage = _image_artifacts.PostprocessedGeneratedImage
_VariantPayload = _image_artifacts.VariantPayload
_compute_blurhash = _image_artifacts.compute_blurhash
_decode_upstream_image_b64 = _image_artifacts.decode_upstream_image_b64
_image_has_alpha = _image_artifacts.image_has_alpha
_image_has_transparency = _image_artifacts.image_has_transparency
_inspect_generated_image_sync = _image_artifacts.inspect_generated_image_sync
_make_display = _image_artifacts.make_display
_make_preview = _image_artifacts.make_preview
_make_thumb = _image_artifacts.make_thumb
_make_variants_with_pil_sync = _image_artifacts.make_variants_with_pil_sync
_make_variants_with_vips_sync = _image_artifacts.make_variants_with_vips_sync
_resize_vips_image = _image_artifacts.resize_vips_image
_rgb_image_for_flat_variant = _image_artifacts.rgb_image_for_flat_variant
_sha256 = _image_artifacts.sha256
_validate_generated_image_metadata = _image_artifacts.validate_generated_image_metadata
_webp_image_for_variant = _image_artifacts.webp_image_for_variant


def _upstream_impl() -> Any:
    from ...upstream_parts import upstream_impl

    return upstream_impl


async def _resolve_image_primary_route() -> str:
    return await _upstream_impl()._resolve_image_primary_route()


def push_image_quota_context(*args: Any, **kwargs: Any) -> Any:
    return _upstream_impl().push_image_quota_context(*args, **kwargs)


def pop_image_quota_context(*args: Any, **kwargs: Any) -> Any:
    return _upstream_impl().pop_image_quota_context(*args, **kwargs)


def push_image_trace_id(*args: Any, **kwargs: Any) -> Any:
    return _upstream_impl().push_image_trace_id(*args, **kwargs)


def pop_image_trace_id(*args: Any, **kwargs: Any) -> Any:
    return _upstream_impl().pop_image_trace_id(*args, **kwargs)


def push_image_retry_attempt(*args: Any, **kwargs: Any) -> Any:
    return _upstream_impl().push_image_retry_attempt(*args, **kwargs)


def pop_image_retry_attempt(*args: Any, **kwargs: Any) -> Any:
    return _upstream_impl().pop_image_retry_attempt(*args, **kwargs)


# The runner receives these dependencies through the explicit generation ports.
...
_PROVIDER_COOLDOWN_LOCAL = _generation_queue.QUEUE_RUNTIME.provider_cooldowns
...
def _make_image_variants_sync(raw_image: bytes) -> _ImageVariantBundle:
    ...
    try:
        return _image_artifacts.make_variants_with_vips_sync(raw_image, inspection)
    except Exception:
        return _image_artifacts.make_variants_with_pil_sync(raw_image, inspection)


def _make_image_variants_pil_only_sync(raw_image: bytes) -> _ImageVariantBundle:
    return _image_artifacts.make_variants_with_pil_sync(raw_image)
...
async def run_generation(ctx: dict[str, Any], task_id: str) -> None:
    """Compatibility wrapper retained for internal task registration.
...
def build_generation_runtime() -> GenerationRuntime:
    ports = GenerationPorts(
        ...
        _IMAGE_QUEUE_AVOID_TTL_S=_IMAGE_QUEUE_AVOID_TTL_S,
        _IMAGE_QUEUE_DEFAULT_LANE=_IMAGE_QUEUE_DEFAULT_LANE,
        _IMAGE_QUEUE_ENQUEUE_DEDUPE_PREFIX=_IMAGE_QUEUE_ENQUEUE_DEDUPE_PREFIX,
        _IMAGE_QUEUE_ENQUEUE_DEDUPE_TTL_S=_IMAGE_QUEUE_ENQUEUE_DEDUPE_TTL_S,
        _IMAGE_QUEUE_FAIR_SCAN_LIMIT=_IMAGE_QUEUE_FAIR_SCAN_LIMIT,
        _IMAGE_QUEUE_LANE_CURSOR_KEY=_IMAGE_QUEUE_LANE_CURSOR_KEY,
        _IMAGE_QUEUE_LANE_RANK=_IMAGE_QUEUE_LANE_RANK,
        _IMAGE_QUEUE_LANE_WEIGHTS=_IMAGE_QUEUE_LANE_WEIGHTS,
        _IMAGE_QUEUE_LOCK_KEY=_IMAGE_QUEUE_LOCK_KEY,
        _IMAGE_QUEUE_LOCK_TTL_S=_IMAGE_QUEUE_LOCK_TTL_S,
        _IMAGE_QUEUE_LOCK_WAIT_S=_IMAGE_QUEUE_LOCK_WAIT_S,
        _IMAGE_QUEUE_NOT_BEFORE_GRACE_S=_IMAGE_QUEUE_NOT_BEFORE_GRACE_S,
        _IMAGE_QUEUE_NOT_BEFORE_PREFIX=_IMAGE_QUEUE_NOT_BEFORE_PREFIX,
        _IMAGE_QUEUE_PROVIDER_LOCK_PREFIX=_IMAGE_QUEUE_PROVIDER_LOCK_PREFIX,
        ...
        _image_queue_avoid_key=_image_queue_avoid_key,
        _image_queue_capacity=_image_queue_capacity,
        ...
        _queued_candidate_from_mapping=_queued_candidate_from_mapping,
        _queued_generation_candidates=_queued_generation_candidates,
