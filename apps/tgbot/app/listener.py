...
- 一个 user 一个 coroutine，串行消费 stream。这样保证 cursor advance 永远
  在 _on_succeeded 完成之后；mid-dispatch crash 重启 replay → delivery lock
  过期后重试，notified key 只在发送完成后置位。
...
async def _user_worker(
    ...
    崩溃」可以靠 replay 找回；发送完成后才写 notified 防止 crash 丢图。
    """
...
async def _on_succeeded(bot: Bot, api: LumenApi, gen_id: str, track, data: dict[str, Any]) -> None:
    ...
    await tracker.mark_notified(gen_id)
