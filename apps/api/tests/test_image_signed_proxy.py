...
async def test_returns_403_for_expired_signature(_signed_proxy_secret: str) -> None:
    # sign_image_url_query() now rejects pathological caller clocks, so build a
    # valid HMAC over an already-expired exp_ms directly for the route check.
    exp_ms = 1
    sig = compute_image_sig(_IMG_ID, "orig", exp_ms, _SECRET.encode("utf-8"))
    with pytest.raises(HTTPException) as exc:
        await images.get_image_signed(
            _IMG_ID,
            "orig",
...
async def test_returns_200_for_valid_orig(
    ...
    resp = await images.get_image_signed(
        ...
        sig,
        _request(),
        _SequencedDb([img, "share-1"]),  # type: ignore[arg-type]
    )
