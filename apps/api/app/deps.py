...
def _unauthorized(
    code: str = "unauthenticated", msg: str = "missing or invalid session"
) -> HTTPException:
...
async def require_bot_token(request: Request) -> None:
    ...
    if not expected or not provided or not hmac.compare_digest(expected, provided):
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={
                "error": {"code": "bot_unauthorized", "message": "invalid bot token"}
            },
        )
...
async def get_bot_user(
    ...
    if not row:
        raise HTTPException(
            ...
            detail={
                "error": {"code": "not_bound", "message": "telegram chat is not bound"}
            },
        )
    _binding, user = row
    request.state.current_user = user
