...
def _clean_str_list(values: Any, *, max_items: int, max_len: int) -> list[str]:
...
def _preset_thumb_storage_key(
    ...
    safe_base = (
        re.sub(r"[^a-zA-Z0-9._-]+", "_", base_sample_name).strip("._-") or "sample"
    )
    safe_suffix = (
        suffix if suffix.startswith(".") else f".{suffix}" if suffix else ".webp"
    )
...
# Public contracts for workflow-owned consumers.
default_sync_mode = _DEFAULT_SYNC_MODE
poster_style_sync_file_lock = _poster_style_sync_file_lock
preset_item_id = _preset_item_id
scan_local_presets = _scan_local_presets
