...
def _showcase_scene_card_direction(scene_card: dict[str, Any] | None) -> str:
    ...
    parts = [
        kv("场景风格", _showcase_scene_label(scene_card.get("scene_family"))),
        ...
        *detail_parts,
        ...
        kv("本张商品呈现", _showcase_scene_label(scene_card.get("product_visibility"))),
        kv("禁令", negative_line),
    ]
...
def _showcase_visibility_policy(
    *,
    ...
    wants_full_body = shot_type in {"front_full_body", "side_or_back"} or _text_has_any(
        text, ("full_body", "全身", "head_to_toe")
    )
    ...
    for item in preserve_items:
        ...
        item_is_front_detail = _text_has_any(item, front_detail_tokens)
        if is_back_or_side:
            if item_is_back_detail:
                visible.append(item)
            elif item_is_front_detail:
                deferred.append(item)
            elif _text_has_any(item, side_visible_tokens):
                visible.append(item)
            elif _text_has_any(item, lower_tokens) and (wants_hem or wants_full_body):
                visible.append(item)
            else:
                deferred.append(item)
            continue
...
def _showcase_occlusion_for_current_view(occlusion: str, deferred_line: str) -> str:
    text = occlusion.strip()
    if not text:
        return ""
    if not _text_has_any(deferred_line, ("前胸", "前片", "胸口", "正面")):
        return text
    text = re.sub(
        r"；?胸前、领口、袖口、口袋、纽扣和图案/logo必须清楚可见。?",
        "",
        text,
    ).strip("；。")
    view_rule = (
        "本张只要求当前角度入镜的领口、袖口、背带/后片、裙摆/下摆和布料纹理清楚；"
        "正面胸口、前片口袋或贴布细节可由其它角度展示"
    )
    return f"{text}；{view_rule}。" if text else f"{view_rule}。"
    ...
    *,
    ...
    for record in prompt_records:
        ...
        if not use_gpt_review:
            ...
            review = _fallback_risk_review(scene_card=scene_card, reason=reason)
...
async def _update_showcase_preflight_progress(
    *,
    db: AsyncSession,
    workflow_run_id: str,
    request_id: str,
    ...
    showcase = await _step(db, workflow_run_id, "showcase_generation")
    if not _showcase_generation_request_matches(showcase, request_id):
        await db.rollback()
        return
    existing = showcase.input_json or {}
    existing_current = existing.get("preflight_phase_current")
    if current is not None:
        try:
            if phase == existing.get("preflight_phase"):
                current = max(int(existing_current), current)
        except (TypeError, ValueError):
            pass
    now = _iso_now()
    ...
    showcase.input_json = {
        **existing,
        ...
        "preflight_phase_detail": detail,
        "preflight_phase_updated_at": now,
        **({"preflight_phase_current": current} if current is not None else {}),
        **({"preflight_phase_total": total} if total is not None else {}),
...
async def _dispatch_showcase_images_generation(
    *,
    ...
    await db.commit()
    progress_total = len(context["shot_picks"])
    await _update_showcase_preflight_progress(
        db=db,
        workflow_run_id=workflow_run_id,
        request_id=request_id,
        phase="director",
        detail=f"正在批量规划并扩写 {progress_total} 张镜头与提示词",
        current=0,
        total=progress_total,
    )

    async def report_preflight_progress(
        phase: str,
        detail: str,
        current: int | None = None,
        total: int | None = None,
        ...
        async with _Session() as progress_db:
            await _update_showcase_preflight_progress(
                db=progress_db,
                workflow_run_id=workflow_run_id,
                request_id=request_id,
    ...
    preflight = await _prepare_showcase_preflight(
        ...
        progress_hook=report_preflight_progress,
    )
