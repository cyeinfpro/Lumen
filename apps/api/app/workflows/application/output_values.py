...
from .values import dedupe_nonempty as _dedupe_nonempty
...
class _QualityReportLike(Protocol):
    score: object
    report_json: object
...
def _failed_generation_output(
    ...
    failed_generations: Iterable[_GenerationLike],
    *,
    fallback: str,
    partial: bool,
) -> dict[str, Any]:
...
def _candidate_generated_image_ids(candidate: _ModelCandidateLike) -> list[str]:
...
def _quality_summary_payload(
    reports: list[_QualityReportLike],
) -> dict[str, Any]:
    if not reports:
